#!/usr/bin/env python3
"""
Crea due immagini, una per accuracy e una per loss, confrontando tre dataset.
...
def grouped_boxplot_with_median_lines(
    ...
    """Crea e salva un boxplot raggruppato per accuracy oppure loss."""
    ...
    legend_handles = []
    for offset, name in zip(offsets, dataset_names):
        ...
        line, = ax.plot(positions, medians, marker="o", linewidth=1.8, label=name)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Crea due immagini boxplot, accuracy e loss, per tre dataset .pt."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--accuracy-name",
        default="accuracy_boxplot_all_datasets.png",
        help="Nome file dell'immagine accuracy.",
    )
    parser.add_argument(
        "--loss-name",
        default="loss_boxplot_all_datasets.png",
